...
# ─────────────────────────────────────────────────────────
# ✅ [핵심 수정] CSV 날짜 파싱 및 단위 변환을 한 번만 수행하는 함수
# 탭마다 반복 변환하는 구조를 제거하고, 로딩 시 한 번만 처리
# ─────────────────────────────────────────────────────────
@st.cache_data
def preprocess_csv(df_raw: pd.DataFrame) -> pd.DataFrame:
    """
    원본 CSV를 받아서:
    1. 날짜 파싱 (연_csv, 월_csv 컬럼 추가)
    2. 사용량 컬럼을 원본 단위 그대로 보존
    3. 단위 변환은 탭별로 뷰 시점에서만 적용
    """
    df = df_raw.copy()
    ...
    if date_col:
        try:
            ...
            mask = parsed.isna()
            if mask.any():
                parsed.loc[mask] = pd.to_datetime(df.loc[mask, date_col], format="%Y%m", errors="coerce")
            mask = parsed.isna()
            if mask.any():
                parsed.loc[mask] = pd.to_datetime(df.loc[mask, date_col], errors="coerce")
            df["날짜_파싱"] = parsed
        except Exception:
            pass

    # 날짜 파싱 실패 시 기본값
    df["날짜_파싱"] = df["날짜_파싱"].fillna(pd.to_datetime("2026-03-01"))
    df["연_csv"] = df["날짜_파싱"].dt.year
    df["월_csv"] = df["날짜_파싱"].dt.month

    return df


def get_unit_col_value(df: pd.DataFrame, unit_str: str) -> pd.Series:
    """
    단위에 따라 사용량 컬럼을 선택하고 단위 변환 적용
    - mj → GJ: /1000
    - m3 → 천m³: /1000
    원본 df는 절대 수정하지 않고 Series만 반환
    """
    if unit_str == "GJ" and "사용량(mj)" in df.columns:
        return pd.to_numeric(df["사용량(mj)"], errors="coerce").fillna(0) / 1000.0
    elif unit_str == "천m³" and "사용량(m3)" in df.columns:
        return pd.to_numeric(df["사용량(m3)"], errors="coerce").fillna(0) / 1000.0
    # fallback
...
# ✅ [수정] CSV 원본을 한 번만 로드 & 날짜 파싱 처리 (단위 변환 없이)
...
if src_csv == "레포 파일 사용":
    ...
    if csv_list: df_csv_raw = pd.concat(csv_list, ignore_index=True)
...
for idx, rpt_tab in enumerate(rpt_tabs):
    with rpt_tab:
        if idx == 0:
            ...
            unit_str = "GJ"
            val_col_raw = "사용량(mj)"   # 원본 컬럼명
            key_sfx = "_gj"
        else:
            df_long_rpt = long_dict_rpt.get("부피", pd.DataFrame())
            unit_str = "천m³"
            val_col_raw = "사용량(m3)"   # 원본 컬럼명
            key_sfx = "_vol"

        # ✅ [수정] 탭별 단위 변환: df_csv_parsed를 복사 후 단위컬럼만 추가
        # 원본 사용량 컬럼은 건드리지 않고, "사용량_단위변환" 컬럼을 새로 추가
        UNIT_VAL_COL = "사용량_단위변환"
        if not df_csv_parsed.empty:
            df_csv_tab = df_csv_parsed.copy()
            df_csv_tab[UNIT_VAL_COL] = get_unit_col_value(df_csv_tab, unit_str)
        else:
            df_csv_tab = pd.DataFrame()

        # 이후 val_col 을 UNIT_VAL_COL 로 사용
        ...
        # ✅ [수정] CSV 기준 최신 연월 반영 (df_csv_tab 에서 안전하게)
        if not df_csv_tab.empty and "날짜_파싱" in df_csv_tab.columns:
            csv_max_date = df_csv_tab["날짜_파싱"].max()
            if pd.notna(csv_max_date):
                if csv_max_date.year not in years_available:
                    years_available = sorted(set(years_available) | {csv_max_date.year})
                    ...
                    if alarm_df.empty:
                        ...
                        if not alarm_df.empty:
                            ...
                            view_state = pdk.ViewState(
                                ...
                                zoom=11 if selected_indices else 11,
                                pitch=40,
                            )
        ...
        def render_full_usage_report(usage_name, section_num, key_sfx, db_key):
            ...
            if not df_csv_tab.empty and val_col in df_csv_tab.columns:
                ...
                if not df_sub_filtered_base.empty and "고객명" in df_sub_filtered_base.columns:
                    ...
                    if sel_cust != "선택 안함":
                        # ✅ [수정] 개별 고객 조회 시에도 df_csv_tab(단위변환 완료본)에서 조회
