...
for idx, rpt_tab in enumerate(rpt_tabs):
    with rpt_tab:
        ...
        if not df_csv_tab.empty:
            ...
            df_csv_tab["연_csv"] = df_csv_tab["날짜_파싱"].dt.year.fillna(2026).astype(int)
            df_csv_tab["월_csv"] = df_csv_tab["날짜_파싱"].dt.month.fillna(3).astype(int)
            
            # 🟢 강력 방어: 무조건 리스트 반환
            try:
                valid_years = sorted(list(set(df_csv_tab["연_csv"].tolist())))
                if valid_years:
                    years_available = valid_years
                    max_year = max(years_available)
                    m_series = df_csv_tab[df_csv_tab["연_csv"] == max_year]["월_csv"]
                    if not m_series.empty:
                        max_month = int(m_series.max())
