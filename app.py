...
for idx, rpt_tab in enumerate(rpt_tabs):
    with rpt_tab:
        ...
        if not df_csv_tab.empty:
            ...
            # 🟢 강력한 방어 코드: 절대 TypeError가 나지 않는 리스트 변환법
            avail_years = [2024, 2025, 2026]
            if "연_csv" in df_csv_tab.columns:
                valid_years = df_csv_tab["연_csv"].dropna().astype(int).tolist()
                if valid_years:
                    avail_years = sorted(list(set(valid_years)))

            years_available = avail_years
            max_year = max(years_available)
            
            max_month = 3
            if "월_csv" in df_csv_tab.columns:
                try:
                    m = df_csv_tab[df_csv_tab["연_csv"] == max_year]["월_csv"].max()
                    if pd.notna(m):
                        max_month = int(m)
                except:
                    pass
