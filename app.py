...
# ─────────────────────────────────────────────────────────
# ✅ [핵심] GitHub Raw URL로 CSV 파일 목록을 가져와 직접 다운로드
# glob 대신 GitHub API로 파일 목록을 확인 후 raw URL로 각각 다운로드
# ─────────────────────────────────────────────────────────
def load_csvs_via_github_api() -> pd.DataFrame:
    """
    GitHub API로 파일 목록을 가져온 뒤,
    각 파일을 Raw URL로 다운로드해서 합칩니다.
    """
    try:
        token = ""
        try:
        ...
        # GitHub API로 루트 디렉토리 파일 목록 가져오기
        ...
        csv_files = [
            ...
            if f["name"].endswith(".csv") and "가정용외" in f["name"]
        ]
        ...
        for f in csv_files:
            ...
            try:
                ...
                if r.status_code == 200:
                    content = load_safe_csv(r.content)
...
if src_csv == "레포 파일 사용":
    ...
    # 2단계: 로컬에 파일 없으면 GitHub API + Raw URL로 다운로드
    if df_csv_raw.empty:
        ...
        if (not df_csv_tab.empty
            ...
            if not map_curr.empty and not map_prev.empty:
                ...
                df_map_merged["증감률(%)"] = np.where(
                    df_map_merged["전년도"] > 0,
                    ((df_map_merged["당해년도"] - df_map_merged["전년도"]) / df_map_merged["전년도"]) * 100, 0)
                ...
                if alarm_df.empty:
                    ...
                    if not alarm_df.empty:
                        ...
                        if editor_key in st.session_state:
                            for idx_str, row_changes in st.session_state[editor_key].get("edited_rows", {}).items():
                        ...
                        if selected_indices:
                            ...
                            start_pt_data = pd.DataFrame([{"lon": start_lon, "lat": start_lat,
                                "tooltip": "<b>🏢 대성에너지 서부지사 (출발지)</b>", "color": [255, 193, 7, 255], "radius": 150}])
                            layers.append(pdk.Layer("ScatterplotLayer", data=start_pt_data,
                                get_position='[lon, lat]', get_color='color', get_radius='radius',
                        ...
                        view_state = pdk.ViewState(
                            ...
                            zoom=11, pitch=40)
                        r = pdk.Deck(map_style=deck_map_style, layers=layers, initial_view_state=view_state,
                        ...
                        total_row = pd.DataFrame([{"선택": False, "No.": "", "용도_태그": "💡 총계",
                            "고객명": "", "도로명주소": "", prev_col_name: sum_prev_all,
                            curr_col_name: sum_curr_all, "증감": sum_curr_all - sum_prev_all,
                            "증감률(%)": sum_rate_all, "비고": ""}])
        ...
        def render_full_usage_report(usage_name, section_num, key_sfx_inner, db_key):
            ...
            if not df_long_rpt.empty:
                ...
                if usage_name == "산업용":
                    ...
                    df_u_csv = df_csv_tab[csv_products.isin(["냉난방용(업무)", "업무난방용", "주한미군"])].copy()
            ...
            if comp_mode == "YoY":
                ...
                prev_name = f"{prev_year}년"; curr_name = f"{curr_year}년"
                diff_label = "전년대비"
                ...
                prev_name = f"전월({prev_month}월)"; curr_name = f"당월({curr_month}월)"
                diff_label = "전월대비"
                vals_prev = []
                for m in range(1, curr_month + 1):
                    if m > 1:
                        ...
                        if not df_long_rpt.empty:
                            p_ly = df_long_rpt[(df_long_rpt["그룹"] == usage_name) & (df_long_rpt["연"] == curr_year-1) & (df_long_rpt["계획/실적"] == "실적")].groupby("월")["값"].sum()
            ...
            overall_max = max(max([sum_prev, sum_act], default=0), max(vals_act, default=0), max(vals_prev, default=0))
            ...
            if not df_csv_tab.empty and val_col in df_csv_tab.columns:
                ...
                if usage_name == "산업용":
                    ...
                    df_sub_base = df_csv_tab[csv_products2.isin(["냉난방용(업무)", "업무난방용", "주한미군"])].copy()
                ...
                if not df_sub_base.empty and grp_col in df_sub_base.columns:
                    ...
                    if len(ind_comp) > 10:
                        ...
                        ind_comp_plot = pd.concat([top10, pd.DataFrame([{grp_col: "기타", prev_name: others[prev_name].sum(), curr_name: others[curr_name].sum()}])], ignore_index=True)
                    else:
                ...
                if not df_sub_base.empty and "고객명" in df_sub_base.columns:
                    ...
                    sel_cust = st.selectbox(f"상세 분석할 고객명을 선택하세요 ({usage_name})", ["선택 안함"] + top_customers, key=f"sel_cust_{usage_name}_{key_sfx_inner}")
                    ...
                    if sel_cust != "선택 안함":
                        ...
                        chart_title = (f"'{sel_cust}' 누적 사용량 ({curr_month}월 누적)" if agg_mode == "누적 실적 (1월~당월)" else f"'{sel_cust}' 당월 사용량 ({curr_month}월 당월)")
                        ...
                        with cc1:
                            ...
                            fig_cust_cum.add_trace(go.Bar(x=[prev_name, curr_name], y=[sum_prev_c, sum_cur_c],
                                marker_color=[COLOR_PREV, COLOR_ACT], text=[f"{sum_prev_c:,.0f}", f"{sum_cur_c:,.0f}"],
                                textposition='auto', hovertemplate="%{x}: %{y:,.0f}<extra></extra>"))
